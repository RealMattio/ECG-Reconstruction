...
class DualBranchHybrid(nn.Module):
    def __init__(self, configs, seq_len=125):
        ...
        # Se usiamo PPG + Derivata + ECG Past = 3 canali raw
        self.raw_input_channels = 3 
        
        if self.apply_wst:
            # Lunghezza attesa per i filtri dello scattering (es. 875)
            ...
            # Calcolo altezza immagine: 3 segnali * coeffs_per_sig
        ...
        self.split_dim = self.img_height // 2 # Per la divisione tra i rami (approssimativa)
        ...
        # RAMO 2: Dual Bi-LSTM
        # ============================================================
        # Dividiamo i canali totali tra PPG-branch (PPG+Diff) e ECG-branch
        self.ppg_split = (self.img_height // 3) * 2 # 2/3 dei canali
        self.ecg_split = self.img_height - self.ppg_split # 1/3 dei canali
    ...
    def forward(self, x):
        ...
        final_input = torch.cat([
            F.interpolate(cnn_w[:, :, -self.target_len:], size=self.target_len, mode='linear'),
            F.interpolate(lstm_w[:, :, -self.target_len:], size=self.target_len, mode='linear')
        ], dim=1)
