...
import torch.nn.functional as F  # <--- IMPORTANTE: Aggiunto per la Huber Loss
...
class Trainer:
    def __init__(self, model, device, configs, preprocessor=None):
        ...
        # Tipo di Loss per l'ampiezza (MAE, HUBER o RMSE)
        ...
        # Pesi delle Loss
        self.alpha = configs.get('morph_loss_weight', 0.4) # Pearson
        self.beta = configs.get('ode_loss_weight', 0.1)    # McSharry ODE Loss
        ...
        # --- PARAMETRI FISICI MODELLO McSHARRY (PINN) OTTIMIZZATI ---
        ...
        self.z0 = 0.0 # Baseline isoelettrica teorica
    ...
    def compute_detailed_losses(self, output, target, theta, omega):
        ...
        # 1. Base Loss (Data-Driven: Ampiezza MAE / HUBER / RMSE)
        weights = 1.0 + self.lambda_peak * torch.abs(target)
        
        if self.base_loss_type == 'HUBER':
            # F.huber_loss unisce L1 e L2: è quadratica per errori piccoli, lineare per errori grandi
        ...
        # 3. McSharry ODE Loss (Physics-Informed: Dinamica ed eliminazione rumore)
    ...
    def fit(self, train_loader, val_loader, epochs=100, patience=15):
        ...
        for epoch in range(epochs):
            ...
            # --- BEST MODEL CHECK E SALVATAGGIO ---
            ...
            # Stampa dinamica basata sul tipo di Loss scelto (MAE o HUBER o RMSE)
            loss_name = self.base_loss_type
            
            log_msg = f"[Epoch {epoch+1:03d} | {time_str} | LR: {current_lr:.2e}] "
            log_msg += f"TRAIN: L {t_m['loss']:.4f}, {loss_name} {t_m['amp_loss']:.4f}, Pear {t_m['pearson']:.4f}, ODE {t_m['ode']:.4f} | "
            log_msg += f"VAL: L {v_m['loss']:.4f}, {loss_name} {v_m['amp_loss']:.4f}, Pear {v_m['pearson']:.4f}, ODE {v_m['ode']:.4f} | {status}"
            print(log_msg)
        ...
        print(f"\n✅ Addestramento Fold completato in {f_mins} minuti e {f_secs} secondi.")
